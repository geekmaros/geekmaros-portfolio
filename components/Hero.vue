<template>
  <section class="bg-regal-purple h-screen py-8 px-5 sm:py-20 sm:px-40">
    <span class="text-regal-blue text-base font-semibold lg:text-xl"
      >Hola, my name is</span
    >
    <h1 class="sm:text-6xl leading-snug text-5xl mt-4 text-white font-bold">
      Abdulrasaq Mustapha.
    </h1>
    <h2 class="sm:text-5xl text-4xl mt-4 text-white leading-snug font-semibold">
      I <span class="text-regal-blue">design</span> &
      <span class="text-regal-pink">develop</span> frontend apps.
    </h2>
    <p
      class="text-white font-normal text-base sm:pr-32 sm:text-xl tracking-normal mt-5 leading-2"
    >
      I'm a Frontend Engineer with over 2 years of experience in designing and
      developing scalable websites and frontend applications.
    </p>
    <a href="mailto:razicruz@gmail.com" class="focus:outline-none">
      <button
        class="text-regal-pink focus:outline-none mt-12 hover:bg-regal-purple hover:text-regal-pink rounded border border-regal-blue px-5 py-3 bg-transparent font-hairline font-medium"
      >
        Get In Touch
      </button>
    </a>
  </section>
</template>
<script>
export default {}
</script>

<style scoped lang="sass">
section
  height: calc(100vh - 5rem)
</style>
